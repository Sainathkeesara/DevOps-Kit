...
```bash
./scripts/bash/k8s_toolkit/pod/restart-pod.sh <pod-name> [--namespace=<ns>] [--grace-period=<seconds>] [--force] [--dry-run]
```
...
3. If standalone, prompts for confirmation before deletion (unless --dry-run)
4. Captures controller information before deletion to monitor rollout
5. Deletes the pod (unless --dry-run)
6. Waits for controller to create a replacement (if applicable)
7. Verifies new pod reaches ready state

**Expected behavior:**
- Managed pods: Controller creates replacement automatically; script waits for rollout to complete.
- Standalone pods: User warned, pod is permanently deleted (no rollback).
- With --dry-run: outputs the deletion command and exits without changes.

**Rollback:**
- For managed pods, roll back to previous revision: `kubectl rollout undo <controller-kind>/<controller-name>`.
- Standalone pods cannot be restored once deleted.
...
```bash
./scripts/bash/k8s_toolkit/report/namespace-report.sh [--namespace=<ns>] [--output=<format>] [--include-events] [--include-metrics]
```
...
- **drain-node**: Node is marked unschedulable (`kubectl get node <node> -o jsonpath='{.spec.unschedulable}'` returns `true`).
- **rollout-status**: Exit code 0 and output indicates success; deployment status shows `available`.
- **restart-pod**: New pod reaches `Running` state; `kubectl get pod <pod>` shows `READY` status. For managed pods, `kubectl rollout status` returns success.
- **pod-logs**: Command exits with code 0 and displays log content.
- **exec-pod**: Command exits with code 0; output of the executed command appears.
- **debug-pod**: Interactive menu appears; selecting commands returns expected output.
- **namespace-report**: Report prints without errors; sections display correct counts and resource usage.

## Rollback

- **drain-node**: Run `kubectl uncordon <node>` to allow scheduling again.
- **restart-pod**: For managed pods, revert to previous revision: `kubectl rollout undo <controller-kind>/<controller-name>`. Standalone pods cannot be restored.
- **Other tools**: Primarily read-only or non-destructive; no rollback needed.
